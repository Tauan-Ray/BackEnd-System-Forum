...
import { PrismaAnswersRepository, PrismaQuestionsRepository, PrismaUserRepository } from "src/infra/database/forum/repositories";
...
export class AnswersService {
    ...
    async updateAnswer(user: userPayload, { id }, updateAnswer: UpdateAnswerDto) {
        ...
        if (existingAnswer.ID_USER !== user.sub && user.role !== "ADMIN") throw new UnauthorizedException('Você não pode alterar a respostas de um outro usuário');
    ...
    async deleteAnswer(user: userPayload, { id }) {
        ...
        if (existingAnswer.ID_USER !== user.sub && user.role !== "ADMIN") throw new UnauthorizedException('Você não pode deletar a respostas de um outro usuário');
